  ...
  componentDidMount() {
    ...
    timer = setInterval(() => {
      ...
      if (notifications.length < reservedNotifications.length) {
        ...
        // object 타입의 key 이름을 notifications로 지정, 저장될 값은 지역변수인 notifications를 사용
  ...
  render() {
    return (
      <div>
        ...
        {/* 자식 컴포넌트인 Notification 를 호출할 수 없어서 화면을 그리지 않음 */}
        {/* setState() 가 실행된 후 render() 함수가 다시 호출됨 */}
        {/* state가 관리하고 있는 배열 notifications 에 데이터가 있으므로 map 함수가 실행됨 */}
        {/* 자식 컴포넌트인 Notification 이 호출 됨 */}
        {this.state.notifications.map((item, index) => {
          // key : 자식 컴포넌트를 반복(map, forEach 등)하여 호출할 경우 리액트는 호출한 자식 컴포넌트를 구분하지 못함
