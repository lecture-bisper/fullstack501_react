...
// 자식 컴포넌트
// 부모 컴포넌트에서 데이터 3가지를 전달받음
// 전달된 데이터는 isLoggedIn, onClickLogin, onClickLogout 3개의 데이터임
function Toolbar(props) {
  // 전개 연산법을 사용하여 부모한테서 전달받은 object 타입의 데이터 props를 한번에 키명과 동일한 변수에 대입함
  const {isLoggedIn, onClickLogin, onClickLogout} = props;

  return (
    <div>
      {/* inline if를 사용하여 화면에 내용 출력 */}
      {/* isLoggedIn 값이 true 이면 && 연산자 뒤의 내용 화면 출력, false 이면 출력 안함 */}
      {isLoggedIn && <span className={'me-3'}>환영합니다.</span>}
      {
        // 삼항 연산자를 사용하여 inline if ~ else를 구현
